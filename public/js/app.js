...
let touchedTotals = document.querySelector(".moves");
function moves() {
	totalMoves++;
	touchedTotals.innerHTML = `Moves: ${totalMoves}`;
}

function canGainLife(bonus) {
	if (bonus % 2 == 0) {
		addLife(lives);
	}
}
...
function addLife(lives) {
	...
	if (totalLives < lifelines) {
		notice.innerHTML = "+1 Life";
		notice.classList.add("fadeUp");
		setTimeout(function () {
			lives[totalLives].src = heartImgs[1];
		}, 1000);
		setTimeout(function () {
			lives[totalLives].src = heartImgs[0];
			totalLives++;
		}, 2000);
	}
}
...
function removelife() {
	...
	if (totalLives >= 0) {
		...
		notice.innerHTML = "-1 Life";
		notice.classList.add("fadeUp");
		let removed = lives[totalLives]; //remove the life from the array
		setTimeout(function () {
...
function resetAll(prevSettings, lifelines) {
	...
	lives = [];
	totalMoves = 0;
	touchedTotals.innerHTML = "Moves: 0";
